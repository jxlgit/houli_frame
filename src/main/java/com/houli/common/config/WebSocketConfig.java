...
public class WebSocketConfig implements WebSocketConfigurer {
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
        ...
        registry.addHandler(webSocketHandler, "/echo").addInterceptors(handshake).setAllowedOrigins("*");
        //部分 不支持websocket的访问链接,允许跨域
        registry.addHandler(webSocketHandler, "/sockjs/echo").addInterceptors(handshake).setAllowedOrigins("*").withSockJS();
    }
}
